...
def run_op(target_op, warmup_count=5, num_measurement=10,
    ...
    with tf.Session(config=config) as sess:
        ...
        _LOGGER.info('Graph execution stats. #samples=%d, median=%s, mean=%s, min=%s, max=%s',
                     len(runtimes),
                     round(np.median(runtimes),4),
                     round(np.average(runtimes),4),
                     round(np.min(runtimes),4),
                     round(np.max(runtimes),4))
...
def generate_cost(target_op, cost_path, sess_config=None, logdir=None):
    ...
    with open('tmp/prof.txt', 'w') as fp:
        for lay, prof in cost_dict.items():
            fp.write(lay.split('/')[-1] + " : ")
            fp.write(str(prof) + "\n")
